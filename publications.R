...
for(y in sort(unique(publish_year))) {
	qqcat("### @{y}\n", file = con)
	qqcat("@{seq_along(author_list)}. @{author_list}, @{titles} <i>@{journal_title}</i> @{y}. <a href='https://www.ncbi.nlm.nih.gov/pubmed/@{unlist(pubmed$IdList)}'>PubMed</a>@{cites2}.</li>\n", file = con)
